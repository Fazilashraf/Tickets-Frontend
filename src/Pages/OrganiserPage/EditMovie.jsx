...
function EditMovie() {
    ...
    const fetchMovie = async () => {
      try {
        ...
        setMovieData({
          ...movie,
          genre: movie.genre || [],
          movPics: movie.movPics || [],
          movieDate: movie.movieDate || [],
          movieTime: movie.movieTime || [],
          cast: movie.cast.length > 0 ? movie.cast : [{ name: '', role: '', image: '' }]
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await editMovieAPI(id, movieData);
  ...
  return (
    <div className="container mt-5 text-light">
      <h2 className="mb-4">✏️ Edit Movie</h2>
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-warning mt-3">Update Movie</button>
      </form>
